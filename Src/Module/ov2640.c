...
void OV2640_TIM_DMA_Init(uint8_t *jpeg_buffer, uint16_t jpeg_buffer_length)
{
    // DMA config
    ...
    DMA_InitStructure.DMA_BufferSize = jpeg_buffer_length;
...
void OV2640_TransmitFrame_JPEG_UART1_TIM_DMA(uint8_t *jpeg_buffer, uint16_t jpeg_buffer_length)
{
    ...
    while (OV2640_VSYNC == 1)
    {
        ...
        {
            TIM3->DIER |= TIM_DMA_CC3; //enable TIM3 DMA request
        }
        if (OV2640_HREF == 0 && (TIM3->DIER & TIM_DMA_CC3) != 0)
        {
            TIM3->DIER &= ~TIM_DMA_CC3; //disable TIM3 DMA request
        }
    }
    uint32_t jpeg_valid_start, jpeg_valid_end = 0;
    for (jpeg_valid_start = 0; jpeg_valid_start < jpeg_buffer_length; jpeg_valid_start++)
    ...
    DMA1_Channel2->CCR |= DMA_CCR2_EN; //enable DMA
}
